...
class TestFirewall(BaseTestCase, unittest.TestCase):
  ...
  def testPublicRule(self):
    ...
    rules = self._query( "SELECT from_node_name, to_node_name, flow_name, service_dst_ports FROM firewall_rule_ip_level WHERE from_node_name = 'DREAMHACK'" )
    self.assertEquals( len(rules), 2, "Wrong number of firewall rules" )

    rule = self._query( "SELECT from_node_name, to_node_name, flow_name, service_dst_ports FROM firewall_rule_ip_level WHERE from_node_name = 'DREAMHACK' AND service_dst_ports = '123/udp,123/tcp'" ) 
  ...
  def testWorldRule(self):
    ...
    self.assertEquals( rule[1], 'www.event.dreamhack.se', "Wrong destination host" )
    self.assertEquals( rule[2], 'normal', "Wrong flow" )
  ...
  def testLocalRule(self):
    ...
    self.assertEquals( rule[3], '77.80.231.128/28', "Wrong source IPv4 address" )
    self.assertEquals( rule[5], 'speedtest1mgmt.event.dreamhack.se', "Wrong destination host" )
    self.assertEquals( rule[11], '69/udp', "Wrong destination port/protocol" )
