def create(conn):
	...
	node_id INTEGER, name TEXT, vlan INTEGER, terminator TEXT, ipv4 INTEGER, ipv4_txt TEXT,
	...
	SELECT node_id, name, ipv4_addr_txt AS ipv4_txt, ipv6_addr_txt AS ipv6_txt FROM host UNION SELECT node_id, name, ipv4_txt, ipv6_txt from network''')
			
	# Create VIEW active_switch
	c.execute('''CREATE VIEW active_switch AS SELECT n.node_id, h.ipv4_addr_txt, n.ipv4_txt, substr(o.value, 1, 1) AS sw, LOWER(n.name) || '-' || substr(o.value, 1, 1) || '.event.dreamhack.local' AS switch_name FROM option o, network n, host h WHERE h.name = switch_name AND o.name = 'sw' AND n.node_id = o.node_id AND sw <> '' UNION SELECT n.node_id, h.ipv4_addr_txt, n.ipv4_txt, substr(o.value, 2, 1) AS sw, LOWER(n.name) || '-' || substr(o.value, 2, 1) || '.event.dreamhack.local' AS switch_name FROM option o, network n, host h WHERE h.name = switch_name AND o.name = 'sw' AND n.node_id = o.node_id AND sw <> '' UNION SELECT n.node_id, h.ipv4_addr_txt, n.ipv4_txt, substr(o.value, 3, 1) AS sw, LOWER(n.name) || '-' || substr(o.value, 3, 1) || '.event.dreamhack.local' AS switch_name FROM option o, network n, host h WHERE h.name = switch_name AND o.name = 'sw' AND n.node_id = o.node_id AND sw <> '';''')
	
	# Create VIEW firewall_rule_ip_level
	c.execute('''CREATE VIEW firewall_rule_ip_level AS
	SELECT fw.is_ipv4, fw.is_ipv6, f.name AS from_node_name, f.ipv4_txt AS from_ipv4, f.ipv6_txt AS from_ipv6, t.name AS to_node_name, t.ipv4_txt AS to_ipv4, t.ipv6_txt AS to_ipv6, fl.name AS flow_name, s.name AS service_name, s.description AS service_description, s.dst_ports AS service_dst_ports, s.src_ports AS service_src_ports FROM node_any f, node_any t, service s, firewall_rule fw, flow fl WHERE s.id = fw.service_id AND fl.id = fw.flow_id AND f.node_id = fw.from_node_id AND t.node_id = fw.to_node_id;''')
