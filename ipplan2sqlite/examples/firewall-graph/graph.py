...
graph = pydot.Dot(graph_type='digraph', splines='true', concentrate='true',
                  pack='true', center='true')
...
for rule in c.fetchall():
    ...
    if from_node == "ANY":
        from_node = "INTERNET"
    first_node = pydot.Node(from_node, fontsize="16.0", shape="box",
                            fillcolor="green", color="#FF0000", style='bold')
    second_node = pydot.Node(nodes[rule[1]], fontsize="16.0", style="filled",
                             fillcolor="green"),
    edge = pydot.Edge(first_node,
                      second_node,
                      label=rule[5],
                      labelfontcolor="#009933", fontsize="10.0",
                      color=services[rule[5]]['color'])
    graph.add_edge(edge)
