...
def add_table(canvas, table, scale=1):
    ...
    canvas.create_rectangle(table.x1, table.y1, table.x2, table.y2, fill="green")

def add_switch(canvas, switch, scale=1):
    size = 2 * scale
    canvas.create_rectangle(switch.x - size, switch.y - size, switch.x + size, switch.y + size, fill="red")

def setup_logging():
    root = logging.getLogger()
    ch = logging.StreamHandler(sys.stdout)
    formatter = logging.Formatter('%(asctime)s - dh-layout - %(levelname)s - %(message)s')
...
def parse_arguments():
    ...
    args_parser.add_argument("--database", help="Path to ipplan SQLite database file", required=True)
...
def get_tables(db, hall):
    sql = "SELECT x1, x2, y1, y2, x_start, y_start, width, height, horizontal FROM table_coordinates WHERE hall = '%s'"
    sql = sql % hall
    return [ Rectangle(*t) for t in db.cursor().execute(sql).fetchall() ]

def get_switches(db, hall):
    sql = "SELECT x, y from switch_coordinates WHERE switch_name LIKE '{}%'".format(hall)
